...
from qgis.PyQt.QtGui import QIcon, QImage, QPainter, QFont, QColor
from qgis.PyQt.QtWidgets import QAction, QGraphicsScene, QApplication, \
        QGraphicsView,QCheckBox, QFileDialog, QTableWidgetItem, QHeaderView
...
class qgisSpectre:
    ...
    def drawspectra(self, data=None):
        ...
        if data is None:
            ...
            # TODO: Possible with selectable colors for dark mode?
       ...
       #     self.scene.addLine(float(n),float(h-(bt+4)-fact*ch),float(n),(h-bt-fact*ch))
            self.scene.addLine(float(ch), float(h - bt - fact * prevvalue), float(ch + 1), 
                               (h - bt - fact * chvalue), spectrepen)
            prevvalue = chvalue
            ch += 1
        self.scene.end = ch - 1
        ...
        ntext.setDefaultTextColor(QColor(0, 0, 0))
    ...
    def peak_finder(self, x, y, window, treshold):
        ...
        # Mariscotti, M. A method for automatic identification of peaks in the presence of
        # background and its application
        ...
        for ch in range(len(s)):
            if peak != []:
                ...
                if s[ch] is None or s[ch] >= 0:
                    ...
                    # Sam Fearn (2022): An Open-Source Iterative python Module for the Automated
                    #                   Identification of Photopeaks in Photon Spectra v2.0.
                    ...
                    # And after the peak, more complicated, since this may be too close
                    # to the end of the spectrum
                    try:
                        post = sum(y[ch:ch + 10]) / 10
                    except TypeError:
                        # TODO: Needs to refine this. This may happen towards the end of the
                        #       spectrum
    ...
    def detectpeaks(self, spectre=None):
        ...
        for (x, ends) in self.peaks:
            ...
            pt.setDefaultTextColor(QColor(0, 0, 0))
            ...
            for e in self.gammas[nuc]:
                ...
                try:
                    for peak in self.peaks:
                        channel = peak[0]
                        draw = draw or (channel - channel * chaccuracy <= x
                                        and channel + channel * chaccuracy >= x)
                ...
                try:
                    xcoord = float(self.scene.left + x)
                    ycoord = 2
                    pl = self.scene.addLine(xcoord, ycoord, xcoord, 300 - self.scene.bottom + 5,
                                            linepen)
    ...
    def setdefault(self):
        ...
        if self.dlg.cbDefault.isChecked():
            s.setValue(f"{self.pluginname}/defaulta", self.scene.acalib)
            s.setValue(f"{self.pluginname}defaultb", self.scene.bcalib)
            s.setValue(f"{self.pluginname}/defaultunit", self.scene.unit)

    def usedefault(self):
        """Fetches default values for the plot """
        s = QgsSettings()
        self.scene.bcalib = s.value(f"{self.pluginname}/defaultb", 0)
        self.scene.acalib = s.value(f"{self.pluginname}/defaulta", 1)
        self.scene.unit = s.value(f"{self.pluginname}/defaultunit", 'Ch')
    ...
    def prepareplot(self):
        ...
        indexbase = f"{self.pluginname}/{layername}_{fieldname}"
        self.scene.acalib = float(s.value(f"{indexbase}_a", s.value(pluginname+"/defaulta", 1)))
        self.scene.bcalib = float(s.value(f"{indexbase}_b", s.value(pluginname+"/defaultb", 0)))
        self.scene.unit = s.value(f"{indexbase}_unit", s.value(pluginname+"/defaultunit", 'Ch'))
    ...
    def findselected(self):
        ...
        if n > 0:
            fieldname = self.dlg.qgField.currentText()
            # DONE: Rewrite to make it possible to read in a spectra as a
    ...
    def saveCalibration(self):
        ...
        fieldname = self.dlg.qgField.currentText()
        s = QgsSettings()
        basename = f'{self.pluginname}/{layername}_{fieldname}'
        s.setValue(f"{basename}_a", self.scene.acalib)
        s.setValue(f"{basename}_b", self.scene.bcalib)
        s.setValue(f"{basename}_unit", self.scene.unit)
    ...
    def run(self):
        ...
        if not self.pluginIsActive:
            ...
            try:
                ...
                # Previous for Qgis3, this is for QGIS4
                self.dlg.qgField.currentIndexChanged.connect(self.prepareplot)
                self.dlg.qgLayer.currentIndexChanged.connect(self.prepareplot)
                self.dlg.leA.textChanged.connect(self.updatecalib)
                self.dlg.leB.textChanged.connect(self.updatecalib)
            self.findselected()
            try:
...
class MouseReadGraphicsView(QGraphicsView):
    ...
    def drawline(self):
        ...
        if unit == 'Ch':
            message = "{} {} (n={})".format(unit, int(energy),
                                            self.spectreval[int(ch)])
        else:
            message = "Ch {}, {} {} (n={})".format(int(ch), int(energy), unit,
                                                   self.spectreval[int(ch)])
        ...
        self.scene().crdtext.setDefaultTextColor(QColor(0, 0, 0))
        self.scene().crdtext.setPos(x, linetop)
        self.scene().markerline = self.scene().addLine(x, linetop, x,
                                                       350 -
                                                       (scene.bottom + 20))
    ...
    def saveImage(self):
        ...
        fileName, _ = QFileDialog.getSaveFileName(self,
                                                  "QFileDialog.getSaveFileName()",   # noqa E501
                                                  "",
                                                  "Image files (*.png);;All Files (*)",  # noqa E501
                                                  options=options)
        if not fileName:
            return
    ...
    def mousePressEvent(self, event):
        """ Press the left mouse button to draw a line and print
            the energy at the point"""

        if event.button() == 1:
            if self.scene() is None or self.scene().left is None:
                # Not yet initialized
                return
            print('Found a scene')
            ...
            if x is not None and x > self.scene().left and x < self.scene().end:  # noqa E501
                self.drawline()
